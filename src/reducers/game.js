...
export function setGameTable(table) {
  ...
  const object = array.reduce((acc, rec, index) => {
    return { ...acc, [index + 1]: 'start' }
  }, {})
  return { type: SET_GAME_TABLE, object }
}
  ...
  return (dispatch, getState) => {
    ...
    if (yellow) {
      return dispatch({
        ...
        gametable: {
          ...gametable,
          [filteredTable[Math.floor(Math.random() * filteredTable.length)]]: 'yellow',
          [yellow]: yellow === greencell ? 'green' : 'red'
        },
        counter: +counter + 1
      })
